 ...
 * @returns {Promise<Got>} Configured Got instance for making HTTP requests
 */
async function getCommerceHttpClient(
  commerceUrl,
  ...
  if (
    (imsOptions && integrationOptions) ||
    (!imsOptions && !integrationOptions)
  ) {
    throw new Error(
      "Either IMS options or integration options must be provided",
    );
  }
...
async function getAdobeCommerceClient(params) {
  ...
  return {
    ...
    getOopePaymentMethod: async (paymentMethodCode) =>
      wrapper(() =>
        commerceGot(`rest/all/V1/oope_payment_method/${paymentMethodCode}`, {
          method: "GET",
        }).json(),
      ),
    ...
    createOopeShippingCarrier: async (shippingCarrier) =>
      wrapper(() =>
        commerceGot(`rest/all/V1/oope_shipping_carrier`, {
          method: "POST",
    ...
    getOopeShippingCarrier: async (shippingCarrierCode) =>
      wrapper(() =>
        commerceGot(
          `rest/all/V1/oope_shipping_carrier/${shippingCarrierCode}`,
          {
            method: "GET",
          },
        ).json(),
    ...
    invoiceOrder: async (orderId) =>
      wrapper(() =>
        commerceGot(`rest/all/V1/order/${orderId}/invoice`, {
          method: "POST",
          json: { capture: true },
        }).json(),
      ),
 ...
 * @param params.__ow_headers request headers
 * @param params.__ow_body request body, requires the following annotation in the action `raw-http: true`
 * @param params.COMMERCE_WEBHOOKS_PUBLIC_KEY the public key to verify the signature configured in the Commerce instance
