...
describe("Admin Main Action", () => {
  ...
  describe("POST requests", () => {
    test("should return 400 if required fields are missing in payload", async () => {
      const params = {
        __ow_method: "post",
        payload: {
          shipstationApiKey: "asbfBDasrqwe",
          warehouseName: "warehouse A",
          warehousePhone: "555-555-5555",
          warehouseAddressLine1: "123 test street",
          warehouseCityLocality: "Springfield",
          warehouseStateProvince: "NA",
          warehousePostalCode: "00000",
          warehouseCountryCode: "USA",
          shipToName: "John Does",
          shipToPhone: "555-555-5556",
        },
      };
    ...
    test("should successfully save configuration and return 200", async () => {
      const mockPayload = {
        shipstationApiKey: "asbfBDasrqwe",
        shipstationCarrierIds: "UPS,USPS,Purolator",
        warehouseName: "warehouse A",
        warehousePhone: "555-555-5555",
        warehouseAddressLine1: "123 test street",
        warehouseCityLocality: "Springfield",
      ...
      expect(body.message).toBe("Saved ShipStation config");
  ...
  describe("GET requests", () => {
    test("should successfully load configuration and return 200", async () => {
      ...
      const body = JSON.parse(response.body);
      expect(body.success).toBe(true);
      expect(body.message).toBe("Loaded ShipStation config"); // Note: The message says "ShipStation config" - you might want to correct this in your action if it should be "ShipStation config"
      expect(body.config).toEqual(mockLoadedConfig);
      expect(mockReadConfiguration).toHaveBeenCalledWith(params, "ShipStation");
      expect(mockLogger.error).not.toHaveBeenCalled();
    });
